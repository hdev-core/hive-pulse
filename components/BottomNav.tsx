...
export const BottomNav: React.FC<BottomNavProps> = ({ currentView, setCurrentView, unreadMessages }) => {
  const navItems = [
    { id: AppView.SWITCHER,      icon: ArrowLeftRight, label: 'Switch' },
  ...
  return (
    <nav className="bg-white border-t border-slate-200 flex justify-between px-1 pt-1.5 pb-2.5 sticky bottom-0 z-30">
      {navItems.map((item) => (
        <button
          key={item.id}
          onClick={() => setCurrentView(item.id)}
          className={`
            flex flex-col items-center gap-0.5 flex-1 py-1 px-0.5 rounded-lg transition-colors relative
            ${currentView === item.id ? 'text-blue-600 bg-blue-50' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'}
          `}
        >
          <item.icon size={17} strokeWidth={currentView === item.id ? 2.5 : 2} />
